...
- RPC(`public.internal_account_*`, SECURITY DEFINER, `search_path=''`, EXECUTE service_role 만): `status(user, session, connection)`, `grant_consent(user, session, policy_version, text_sha256, via)`(K 필요, 정책·해시 일치, 같은 활성 grant 멱등, 구버전 grant 는 대체), `revoke_consent(user, session, grant)`, `register_connection(user, session, source_app, source_mode, platform, label, dataset_key, secret_sha256, takeover)`(동일 dataset 활성 writer 있으면 `writer_conflict`, takeover=true 면 이전 `superseded`), `rebind_connection(user, session, connection, secret_sha256)`(같은 user·비밀·active 만, epoch 유지, 타인/없음은 동일 `not_found`), `revoke_connection(user, connection)`, 삭제는 map migration(5)의 `public.internal_community_delete_contributions(user, session)` 을 account 함수가 부른다(의존 방향). `revoke_consent` 는 주어진 grant 의 **계보의 활성 grant** 를 철회(N-04).
...
### 3.4 map 신규(5) — ingest·fact·공개 (정본: 이 절 = SQL 초안 `stack-poc/poc-sql/v2/202609260200_community_ingest.sql`, 구현 때 migration 으로 이동)
...
- `public.internal_community_ingest(user, session, request_id, envelope, events)`: 잠금 policy_current(SHARE) → contributor(SHARE) → grant(SHARE) → **connection(UPDATE)** → fact(UPDATE) → analytics_state. 권한 재확인 실패는 요청 전체 오류·쓰기 0. 이벤트별: epoch 불일치 rejected, tombstone 또는 삭제 fence 이전 captured_at 은 rejected:deleted, 원장 insert(on conflict: 모든 불변 필드 같으면 duplicate, 아니면 conflict), `quarantine_reason` 있으면 quarantined, fact 비교((epoch, revision) 사전식) → accepted/no_change/stale_ignored. grant 귀속은 `reshare` 이거나 기존 계보가 활성일 때만 새 grant 로(S-02). ACK projection_status = published/removed/held/not_public/not_applicable(계약 observation.md). 변경 있으면 dataset_version·source_updated_at·generated_at 갱신(같은 트랜잭션).
- `public.internal_community_manifest(user, session, connection, after, limit≤5000)`: completed fact key 앞 24hex 페이지 + total + manifest_token.
- `public.internal_community_delete_contributions(user, session)`(auth `community-account` 가 호출): 연결 전부 revoked(contributions_deleted), 삭제 fence, 신고 identity tombstone, fact 삭제, version 갱신. 이벤트 원장은 비공개로 30일 보관 후 운영 정리.
- 공개 RPC 교체 `internal_analytics_v2_facts`(같은 서명·반환 형태): ingest fact 만, completed ∧ 계보 활성 ∧ contributor active, 좌표 결측 포함(bbox 지정 시 좌표 있는 행만). `fact_identity = dataset_key:source_report_key`, `snapshot_id='ingest-v1'`, `snapshot_generation=1`.
- `private.invalidate_analytics_v2()` 교체: contributor 변경 → version·generated_at 만 갱신(ready 유지), 구 snapshot state 변경 → 기존 의미(ready=false).
- `server/aggregate.ts`·`publicHandler.ts`: lat/lng nullable, 지점·bbox 는 좌표 있는 fact 만, `coverage` 에 위치 결측 수, meta 모집단 `shared_completed_reports` 문구, 분모 없는 비율 unsupported.
- 한도 RPC `internal_community_ingest_rate_limit(bucket, limit)` — 사용자·연결 버킷만, IP 헤더 미사용(S-18).
...
## 17. 4차 재확인(plan-review-sol-04) 반영 요약
S-02-R: 삭제 fence + 삭제 시 연결 전부 폐기 + 로컬 대기 행 blocked. S-09-R: ingest 가 connection 을 UPDATE 잠금(병렬 40요청 교착 0 확인). S-11-R: ACK = published/removed/held/not_public/not_applicable, 공개 RPC 와 같은 판정 함수.
N-04: 철회는 계보의 활성 grant 에 귀결. S-10-D: preflight 동적 SQL(표 부재 안전), §3.4 를 SQL 기준으로 재작성. S-03-F: 재시도 의도를 capture 전에 기록, 기록 실패 시 즉시 중단. S-04-M: manifest_token 으로 페이지 일관성 확인·최대 3회 재시도.
