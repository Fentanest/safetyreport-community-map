...
| 공개 | fact 는 **수락한 grant 가 활성**일 때만 공개(S-02). 재동의 자동 재공개 없음, 명시적 `reshare` 만. 삭제 = fact 삭제 + tombstone. 좌표 결측 fact 도 통계에 포함, 지점에서만 제외(S-01). contributor 단위 authoritative 전환으로 구 v2 와 이중 집계 방지(S-10) |
...
[모바일 새 설치] 로컬 schema 준비(사용자 데이터 무변경) → 로딩/게이트 셸(기존 화면 렌더 안 함)
  → [필수]카카오 인증 + [필수]신고내용 공유 동의 → 기존 SetupScreen(모드·서버/공식 계정)
  → 기존 PermissionScreen(모드별 권한) → 초기화 안내·확인(Standalone: 로컬 job / Client: 서버 job) → 메인
...
- 잠금 순서(모든 RPC 공통, S-09): **policy → contributor_profiles → grant → connection → (map) fact → analytics_state**. 계정 변경 RPC 는 contributor 행 FOR UPDATE(사용자당 뮤텍스), ingest 는 FOR SHARE. 운영자 policy 변경은 policy FOR UPDATE.
...
- `private.community_fact_tombstones`(contributor_id, deleted_at, request_session_id) — 삭제 요청 시각. 그 이전 `captured_at` 이벤트는 이후 영구 `rejected: deleted`.
- `private.community_contributor_sources`(contributor_id pk, authoritative='ingest', switched_at) — 첫 ingest 수락 때 기록(S-10).
...
- 공개 RPC 교체 `internal_analytics_v2_facts`(같은 서명·반환 형태):
  - ingest 소스: `public_state='completed'` ∧ fact 의 grant 활성(revoked_at null) ∧ contributor active. **좌표 결측도 포함**(lat/lng null); bbox 인자가 있으면 좌표 있는 행만.
  - 구 v2 소스: 기존 active snapshot 조건 ∧ `community_contributor_sources` 에 없는 contributor 만.
  - 반환: 기존 PrivateFact 형태(`fact_identity`=`dataset_key:source_report_key`, `snapshot_id`=ingest 는 `ingest-v1`, `snapshot_generation`=1), lat/lng nullable.
...
- `private.invalidate_analytics_v2()` 교체: contributor 행 변경 → ready 유지, version·generated_at 갱신. 구 snapshot state 변경 → 기존대로 ready=false(구 경로 보존).
...
- `context` 에 `dataset_key`, `writer_epoch`, `connection_secret`(연결 비밀 원문 — 이 파일은 사용자 권한 파일; PC 는 기존 `data/auth` 암호화 저장소에 비밀을 두고 context 에는 참조만, 모바일은 secure storage) — **비밀 원문은 community.db 에 두지 않는다**.
- `rotate_dataset(reason)`: 새 local_dataset_id, `meta.dataset_history` 에 이전 id·사유·시각. 이전 journal/outbox 는 삭제하지 않고 원 귀속 그대로(현재 context 와 다르면 전송 대상 아님).
...
| T5 모바일 게이트·초기화 | **Muse** | `safetyreport-mobile/gate-rebuild` · `ci0926/gate-rebuild` | `lib/main.dart`, `lib/providers/report_provider.dart`, `lib/community/gate/**`, `lib/community/rebuild/**`, `lib/screens/community_onboarding_screen.dart`, `lib/screens/community_rebuild_screen.dart`, `lib/services/community_server_link_service.dart`, `lib/services/server_contract.dart`, `lib/widgets/community_account_card.dart`, `ios/Runner/Info.plist`(URL types), `ios/Runner/AppDelegate.swift`, 테스트 | T0b |
