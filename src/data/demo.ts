...
export function demoDashboard(scope: Scope, state: 'overview' | 'one' | 'empty' = 'overview'): DashboardData {
  ...
  const R = empty ? 0 : one ? 1 : 9;
  const C = empty ? 0 : one ? 1 : 7;
  const D = empty ? 0 : one ? 1 : 6;
  const accepted = empty ? 0 : one ? 1 : 4;
  const partial = empty || one ? 0 : 1;
  const rejected = empty || one ? 0 : 1;
  ...
  const rate: RateMetric = available
    ? { ...count(D ? (accepted + partial) / D * 100 : null, 'completed_date', D), numerator: accepted + partial, unit: 'percent', eligible: D, missing: C - D }
